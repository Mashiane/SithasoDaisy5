...
'	chartnames.Responsive = True
	...
	db.Initialize(Me, "db", Main.ServerDB, "contacts")
	'db.ShowLog = True
	...
	db.Initialize(Me, "db", Main.ServerDB, "provinces")
	db.SetSchemaFromDataModel(app.DataModels)
	...
	db.Initialize(Me, "db", Main.ServerDB, "contacts")
	db.SetSchemaFromDataModel(app.DataModels)
	'get the id and gender for all contacts
	db.selectFields(Array("id", "gender"))
	...
	db.SetSchemaFromDataModel(app.DataModels)
	'get the id and gender for all contacts
	db.selectFields(Array("id", "letter"))
	'group by gender, count records per gender and order output by gender
	BANano.Await(db.GroupBy(Array("letter"), Array("count|*|records"), Array("letter")))
	chartnames.Clear
	Do While db.NextRow
