...
Private Sub MountContacts(bAfterChange As Boolean)
	...
	db.InitializeOnConnection(Me, Main.PocketBase, "contacts", Main.PocketBaseURL, "contacts")
...
private Sub EditMode(item As Map)
	...
	Dim sAbbreviation As String = item.Get("abbreviation")
	Dim sFullname As String = item.Get("fullname")
	Dim sLetter As String = item.Get("letter")
	...
	Dim sGender As String = item.Get("gender")
...
Private Sub LoadCategories				'ignore
	...
	Dim db As SDUIPocketBase
	db.InitializeOnConnection(Me, Main.PocketBase, "categories", Main.PocketBaseURL, "categories")
	'link this api class to the data models
	db.SetSchemaFromDataModel(App.DataModels)
	'db.ShowLogs = True
	'clear any where clauses
...
Private Sub LoadProvinces				'ignore
	...
	Dim db As SDUIPocketBase
	db.InitializeOnConnection(Me, Main.PocketBase, "provinces", Main.PocketBaseURL, "provinces")
	'link this api class to the data models
	db.SetSchemaFromDataModel(App.DataModels)
	'db.ShowLogs = True
	'clear any where clauses
...
Private Sub cboCategory_append (e As BANanoEvent)
	...
	Dim tblX As SDUIPocketBase
	tblX.InitializeOnConnection(Me, Main.PocketBase, "categories", Main.PocketBaseURL, "categories")
	'tblX.ShowLogs = True
...
Private Sub cboProvince_append (e As BANanoEvent)
	...
	'define the database connection
	Dim tblX As SDUIPocketBase
	tblX.InitializeOnConnection(Me, Main.PocketBase, "provinces", Main.PocketBaseURL, "provinces")
	'tblX.ShowLogs = True
	'set the schema from the data model
	tblX.SetSchemaFromDataModel(App.DataModels)
	'ensure that there are no duplicates
