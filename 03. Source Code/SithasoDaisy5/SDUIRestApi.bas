...
'Dim pb As SDUIRestApi
'pb.Initialize(Me, "pb", "http://localhost:8080", "projects")
...
Public Sub InitializeApiKey(Module As Object, eventName As String, url As String, sTableName As String, apiKey As String)
	TableName = sTableName
	mCallBack = Module
	whereField.Initialize
	ops.Initialize
...
Public Sub CreateTable As String
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.CREATE_TABLE"$)
	End If
...
Sub SchemaAddLongText1(b As String) As SDUIRestApi
	Schema.Put(b, DB_LONGTEXT)
	Return Me
End Sub

Sub SchemaAddLongText(bools As List) As SDUIRestApi
...
Sub SetLimit(s As String)
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.SetLimit(${s})"$)
	End If
	sSize = s
End Sub
...
'	If ShowLog Then
'		Log($"SDUIRestApi.${mEvent}.setApiKey(${s})"$)
'	End If
'	sApiKey = s
'End Sub

'prepare for new table definition
Sub SchemaClear As SDUIRestApi
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.SchemaClear"$)
	End If
	Schema.clear
	Return Me
End Sub
...
'schema add boolean
Sub SchemaAddBoolean(bools As List) As SDUIRestApi
	For Each b As String In bools
		Schema.Put(b, DB_BOOL)
	Next
	Return Me
End Sub
Sub SchemaAddBoolean1(b As String) As SDUIRestApi
	Schema.Put(b, DB_BOOL)
	Return Me
End Sub
Sub SchemaAddDouble1(b As String) As SDUIRestApi
	Schema.Put(b, DB_DOUBLE)
	Return Me
End Sub
'add double fields
Sub SchemaAddDouble(bools As List) As SDUIRestApi
	For Each b As String In bools
		Schema.Put(b, DB_DOUBLE)
	Next
	Return Me
End Sub
Sub SchemaAddFloat1(b As String) As SDUIRestApi
	Schema.Put(b, DB_FLOAT)
	Return Me
End Sub
Sub SchemaAddFloat(bools As List) As SDUIRestApi
	For Each b As String In bools
		Schema.Put(b, DB_FLOAT)
	Next
	Return Me
End Sub
Sub SchemaAddText1(b As String) As SDUIRestApi
	Schema.Put(b, DB_TEXT)
	Return Me
End Sub
Sub SchemaAddText(bools As List) As SDUIRestApi
	For Each b As String In bools
		Schema.Put(b, DB_TEXT)
	Next
	Return Me
End Sub
Sub SchemaAddInt1(b As String) As SDUIRestApi
	Schema.Put(b, DB_INT)
	Return Me
End Sub
Sub SchemaAddInt(bools As List) As SDUIRestApi
	For Each b As String In bools
		Schema.Put(b, DB_INT)
	Next
	Return Me
...
Sub SetSchemaFromDataModel1(sourceTable As String, models As Map)
	If ShowLog Then
		Log($"SDUIRestApi.${sourceTable}.SetSchemaFromDataModel"$)
	End If
...
Sub SetSchemaFromDataModel(models As Map)
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.SetSchemaFromDataModel"$)
	End If
...
Sub SchemaAddBlob(bools As List) As SDUIRestApi
	For Each b As String In bools
...
Sub SchemaAddFile(bools As List) As SDUIRestApi
	For Each b As String In bools
...
Sub SchemaAddFile1(b As String) As SDUIRestApi
	Schema.Put(b, DB_FILE)
	Return Me
End Sub

Sub SchemaAddBlob1(b As String) As SDUIRestApi
	Schema.Put(b, DB_BLOB)
	Return Me
End Sub

Sub SchemaAddNumber(bools As List) As SDUIRestApi
	For Each b As String In bools
...
Sub SchemaAddNumber1(b As String) As SDUIRestApi
	Schema.Put(b, DB_NUMBER)
	Return Me
End Sub
...
Sub SELECT_ALL As List
	result.Initialize
	If ShowLog Then
		'Log($"SDUIRestApi.${TableName}.SELECT_ALL"$)
	End If
	If Schema.Size = 0 Then
		BANano.Throw($"SDUIRestApi.SELECT_ALL. Please execute SchemaAdd?? first to define your table schema!"$)
		Return result
	End If
	If UseApiKey Then
		If sApiKey = "" Then
			BANano.Throw($"SDUIRestApi.SELECT_ALL - The ApiKey has not been specified!"$)
			Return result
		End If
	End If
	'
...
'	fetch.NoCors = True
	If scommand = "" Then
		fetch.SetURL($"/api/records/${TableName}"$)
		If ShowLog Then
			'Log($"SDUIRestApi.SELECT_ALL.(${baseURL}/api/records/${TableName}?${scommand})"$)
		End If
	Else
		fetch.SetURL($"/api/records/${TableName}?${scommand}"$)
		If ShowLog Then
			'Log($"SDUIRestApi.SELECT_ALL.(${baseURL}/api/records/${TableName}?${scommand})"$)
		End If
	End If
...
Sub CREATE As Map
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.CREATE"$)
	End If
	If Schema.Size = 0 Then
		BANano.Throw($"SDUIRestApi.CREATE. Please execute SchemaAdd?? first to define your table schema!"$)
		Return Null
	End If
	If UseApiKey Then
		If sApiKey = "" Then
			BANano.Throw($"SDUIRestApi.CREATE - The ApiKey has not been specified!"$)
			Return Null
		End If
	End If
	Try
		...
		If pkValue = "" Then
			If ShowLog Then
				Log($"SDUIRestApi.${TableName}.CREATE Error - No Primary Key Found"$)
			End If
			Return Null
		End If
		...
		fetch.NoCache = True
		If ShowLog Then
			Log($"SDUIRestApi.CREATE.(${baseURL}/api/records/${TableName}?${BANano.ToJson(Record)})"$)
		End If
'		fetch.NoCors = True
		...
		Return output
	Catch
		If ShowLog Then Log($"CREATE: ${LastException}"$)
		Return Null
	End Try
End Sub
...
'	If ShowLog Then
'		Log($"SDUIRestApi.${mEvent}.USER_CREATE"$)
'	End If
'	ErrorMessage = ""
'	Try
'		Record.Initialize
...
'	If ShowLog Then
'		Log($"SDUIRestApi.${mEvent}.USER_CHANGE_PASSWORD"$)
'	End If
'	ErrorMessage = ""
'	Try
'		Record.Initialize
...
'	If ShowLog Then
'		Log($"SDUIRestApi.${mEvent}.USER_LOGIN"$)
'	End If
'	ErrorMessage = ""
'	Try
'		Record.Initialize
...
'	If ShowLog Then
'		Log($"SDUIRestApi.${mEvent}.USER_LOGOUT"$)
'	End If
'	ErrorMessage = ""
'	Try
'		Dim fetch As SDUIFetch
...
Sub DELETE(id As String) As Boolean
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.DELETE(${id})"$)
	End If
	If Schema.Size = 0 Then
		BANano.Throw($"SDUIRestApi.DELETE. Please execute SchemaAdd?? first to define your table schema!"$)
		Return False
	End If
	If UseApiKey Then
		If sApiKey = "" Then
			BANano.Throw($"SDUIRestApi.DELETE - The ApiKey has not been specified!"$)
			Return False
		End If
	End If
	Try
		...
		fetch.SetContentTypeApplicationJSON
		If UseApiKey Then fetch.AddHeader("X-API-Key", sApiKey)
		fetch.SetURL($"/api/records/${TableName}/${id}"$)
		fetch.NoCache = True
		If ShowLog Then
			Log($"SDUIRestApi.DELETE.(${baseURL}/api/records/${TableName}/${id})"$)
		End If
'		fetch.NoCors = True
...
Sub DELETE_MULTIPLE(items As List) As Boolean
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.DELETE_MULTIPLE()"$)
	End If
	If Schema.Size = 0 Then
		BANano.Throw($"SDUIRestApi.DELETE_MULTIPLE. Please execute SchemaAdd?? first to define your table schema!"$)
		Return False
	End If
	Try
...
Sub UPDATE As String
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.UPDATE"$)
	End If
	If Schema.Size = 0 Then
		BANano.Throw($"SDUIRestApi.UPDATE. Please execute SchemaAdd?? first to define your table schema!"$)
		Return ""
	End If
	If UseApiKey Then
		If sApiKey = "" Then
			BANano.Throw($"SDUIRestApi.UPDATE - The ApiKey has not been specified!"$)
			Return ""
		End If
	End If
	Try
		...
		If pkValue = "" Then
			If ShowLog Then
				Log($"SDUIRestApi.${TableName}.UPDATE Error - No Primary Key Found"$)
			End If
			Return ""
		End If
		Dim output As Object
		...
		If UseApiKey Then fetch.AddHeader("X-API-Key", sApiKey)
		'this is a post
		fetch.SetURL($"/api/records/${TableName}/${pkValue}"$)
		RemoveField(AutoIncrement)
		fetch.SetData(Record)
		fetch.NoCache = True
		If ShowLog Then
			Log($"SDUIRestApi.UPDATE.(${baseURL}/api/records/${TableName}/${pkValue}?${BANano.ToJson(Record)})"$)
		End If
'		fetch.NoCors = True
...
Sub UPDATE_WHERE As String
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.UPDATE_WHERE"$)
	End If
	If Schema.Size = 0 Then
		BANano.Throw($"SDUIRestApi.UPDATE_WHERE. Please execute SchemaAdd?? first to define your table schema!"$)
		Return ""
	End If
	If UseApiKey Then
		If sApiKey = "" Then
			BANano.Throw($"SDUIRestApi.UPDATE_WHERE - The ApiKey has not been specified!"$)
			Return ""
		End If
	End If
	Try
		...
		If qfilter = "" Then
			fetch.SetURL($"/api/records/${TableName}"$)
		Else
			fetch.SetURL($"/api/records/${TableName}?${qfilter}"$)
		End If
		RemoveField(AutoIncrement)
		fetch.SetData(Record)
		fetch.NoCache = True
		If ShowLog Then
			Log($"SDUIRestApi.UPDATE_WHERE.(${baseURL}/api/records/${TableName}/${qfilter}?${BANano.ToJson(Record)})"$)
		End If
'		fetch.NoCors = True
		BANano.Await(fetch.PutWait)
		If fetch.Success Then
...
Sub READ(id As String) As Map
	...
	Record = m
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.READ(${id})"$)
	End If
	If Schema.Size = 0 Then
		BANano.Throw($"SDUIRestApi.READ. Please execute SchemaAdd?? first to define your table schema!"$)
		Return m
	End If
	If UseApiKey Then
		If sApiKey = "" Then
			BANano.Throw($"SDUIRestApi.READ - The ApiKey has not been specified!"$)
			Return m
		End If
	End If
	Dim m As Map = CreateMap()
...
'		fetch.NoCors = True
		If ShowLog Then
			Log($"SDUIRestApi.READ.(${baseURL}/api/records/${TableName}/${id})"$)
		End If
		BANano.Await(fetch.GetWait)
		If fetch.Success Then
			Dim Response As Map = fetch.response
...
Sub READ_BY(fldName As String, fldValue As Object) As Map
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.READ_BY(${fldName},${fldValue})"$)
	End If
	CLEAR_WHERE
...
Sub READ_BY_STRING(fldName As String, fldValue As Object) As Map
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.READ_BY_STRING(${fldName},${fldValue})"$)
	End If
	CLEAR_WHERE
	ADD_WHERE(fldName, "=", fldValue)
	Dim res As List = BANano.Await(SELECT_WHERE)
...
Sub READ_ID_BY_STRING(fldName As String, fldValue As String) As String
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.READ_ID_BY_STRING(${fldName},${fldValue})"$)
	End If
	CLEAR_WHERE
...
Sub READ_ID_BY(fldName As String, fldValue As String) As String
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.READ_ID_BY(${fldName},${fldValue})"$)
	End If
	CLEAR_WHERE
	ADD_FIELD(PrimaryKey)
	ADD_WHERE_STRING(fldName, "=", fldValue)
...
Sub ContainsKey(id As String) As Boolean
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.ContainsKey(${id})"$)
	End If
	If Schema.Size = 0 Then
		BANano.Throw($"SDUIRestApi.ContainsKey. Please execute SchemaAdd?? first to define your table schema!"$)
		Return ""
	End If
	CLEAR_WHERE
	ADD_FIELD(PrimaryKey)
...
Sub ContainsField(fldName As String, id As String) As Boolean
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.ContainsField(${fldName},${id})"$)
	End If
	If Schema.Size = 0 Then
		BANano.Throw($"SDUIRestApi.ContainsField. Please execute SchemaAdd?? first to define your table schema!"$)
		Return ""
	End If
	CLEAR_WHERE
	ADD_FIELD(fldName)
...
Public Sub ListKeys As List
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.ListKeys"$)
	End If
	If Schema.Size = 0 Then
		BANano.Throw($"SDUIRestApi.ListKeys. Please execute SchemaAdd?? first to define your table schema!"$)
		Return Null
	End If
	CLEAR_WHERE
	ADD_FIELD(PrimaryKey)
...
Sub DELETE_ALL
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.DELETE_ALL"$)
	End If
...
Sub GetKeyValues(bHasNothing As Boolean, k As String, v As String) As Map
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.GetKeyValues(${bHasNothing},${k},${v})"$)
	End If
...
Sub CREATE_OR_UPDATE As String
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.CREATE_OR_UPDATE"$)
	End If
	If Schema.Size = 0 Then
		BANano.Throw($"SDUIRestApi.CREATE_OR_UPDATE. Please execute SchemaAdd?? first to define your table schema!"$)
		Return ""
	End If
	Try
		...
		If Key <> "" Then
			...
			If ShowLog Then
				Log($"SDUIRestApi.CREATE_OR_UPDATE:ContainsKey(id,${Key},${bThere})"$)
			End If
		End If
		'does not exit
...
Sub CREATE_OR_UPDATE_BY_FIELD(fldName As String) As String
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.CREATE_OR_UPDATE_BY_FIELD(${fldName})"$)
	End If
	If Schema.Size = 0 Then
		BANano.Throw($"SDUIRestApi.CREATE_OR_UPDATE_BY_FIELD. Please execute SchemaAdd?? first to define your table schema!"$)
		Return ""
	End If
	Try
		'does the record exist
...
Sub GET_ID_BY_FIELD(fldName As String, fldValue As String) As String
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.GET_ID_BY_FIELD(${fldName},${fldValue})"$)
	End If
...
Sub UPDATE_BY_STRING(fldName As String, fldValue As String) As String
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.UPDATE_BY_STRING(${fldName},${fldValue})"$)
	End If
...
Sub UPDATE_BY(fldName As String, fldValue As String) As String
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.UPDATE_BY(${fldName},${fldValue})"$)
	End If
	Dim rec As Map = BANano.Await(READ_BY(fldName, fldValue))
	Dim sid As String = rec.GetDefault(PrimaryKey, "")
	Record.Put(PrimaryKey, sid)
...
Sub SetField(fldName As String, fldValue As Object) As SDUIRestApi
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.SetField(${fldName}, ${fldValue})"$)
	End If
...
Sub RemoveField(fldName As String)
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.RemoveField(${fldName})"$)
	End If
...
'clear where clause
Sub CLEAR_WHERE As SDUIRestApi
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.CLEAR_WHERE"$)
	End If
	whereField.Initialize
	ops.Initialize
	orderByL.Initialize
...
Sub Reset As SDUIRestApi
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.Reset"$)
	End If
	whereField.Initialize
	ops.Initialize
	orderByL.Initialize
...
Sub ADD_FIELD(fld As String) As SDUIRestApi
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.ADD_FIELD(${fld})"$)
	End If
...
'join on table
Sub ADD_JOIN(fld As String) As SDUIRestApi
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.ADD_JOIN(${fld})"$)
	End If
	joinL.Add($"join=${fld}"$)
	Return Me
End Sub

Sub ADD_FIELDS(fields As List) As SDUIRestApi
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.ADD_FIELDS(${BANano.ToJson(fields)})"$)
	End If
...
'sw - starts with
'ew - ends with
Sub ADD_WHERE(fld As String, operator As String, value As Object) As SDUIRestApi
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.ADD_WHERE(${fld},${operator},${value})"$)
	End If
...
Sub ADD_WHERE_STRING(fld As String, operator As String, value As Object) As SDUIRestApi
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.ADD_WHERE_STRING(${fld},${operator},${value})"$)
	End If
...
Sub WHERE_OR(pos As Int) As SDUIRestApi
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.WHERE_OR(${pos})"$)
	End If
	combineL.Set(pos, "2")
	Return Me
End Sub
...
Sub WHERE_AND(pos As Int) As SDUIRestApi
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.WHERE_AND(${pos})"$)
	End If
	combineL.Set(pos, "")
	Return Me
End Sub
...
'add a sort field
Sub ADD_ORDER_BY(fld As String) As SDUIRestApi
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.ADD_ORDER_BY(${fld})"$)
	End If
	orderByL.Add($"order=${fld}"$)
	Return Me
End Sub

Sub ADD_ORDER_BY_DESC(fld As String) As SDUIRestApi
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.ADD_ORDER_BY_DESC(${fld})"$)
	End If
...
Sub SetRecord(rec As Map)
	...
	DisplayValue = Record.GetDEfault(DisplayField, "")
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.SetRecord(${BANano.ToJson(Record)})"$)
	End If
End Sub
...
Sub DELETE_BY(fldName As String, fldValue As String)
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}DELETE_BY(${fldName},${fldValue})"$)
	End If
	CLEAR_WHERE
	ADD_FIELD(fldName)
	ADD_WHERE(fldName, "=", fldValue)
...
Sub DELETE_BY_STRING(fldName As String, fldValue As String)
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.DELETE_BY_STRING(${fldName},${fldValue})"$)
	End If
	CLEAR_WHERE
	ADD_FIELD(fldName)
	ADD_WHERE(fldName, "=", fldValue)
...
Sub DELETE_WHERE
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.DELETE_WHERE"$)
	End If
	BANano.Await(deleteWhere(PrimaryKey, whereField, ops))
End Sub
...
Sub ExistsByString(fldName As String, fldValue As Object) As Boolean
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.ExistsByString(${fldName},${fldValue})"$)
	End If
	CLEAR_WHERE
	ADD_FIELD(fldName)
	ADD_WHERE_STRING(fldName, "=", fldValue)
...
Sub Exists(fldName As String, fldValue As Object) As Boolean
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.Exists(${fldName},${fldValue})"$)
	End If
	CLEAR_WHERE
	ADD_FIELD(fldName)
	ADD_WHERE(fldName, "=", fldValue)
...
Sub ExistsWhere(where As Map) As Boolean
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.ExistsWhere(${BANano.ToJson(where)})"$)
	End If
	CLEAR_WHERE
	ADD_FIELD(PrimaryKey)
...
Sub SELECT_WHERE As List
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.SELECT_WHERE"$)
	End If
...
Sub SELECT_WHERE1 As List
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.SELECT_WHERE1"$)
	End If
...
Sub PrepareRecord
	If ShowLog Then
		Log($"SDUIRestApi.${mEvent}.PrepareRecord"$)
	End If
	Record.Initialize
	CLEAR_WHERE
End Sub
...
Sub deleteWhere(priKey As String, whereMap As Map, whereOps As List) As Boolean
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.deleteWhere(${priKey},${BANano.ToJson(whereMap)},${whereOps})"$)
	End If
	If Schema.Size = 0 Then
		BANano.Throw($"SDUIRestApi.deleteWhere. Please execute SchemaAdd?? first to define your table schema!"$)
		Return Null
	End If
	'get all the records
...
Sub findWhere(whereMap As Map, whereOps As List) As List
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.findWhere(${BANano.ToJson(whereMap)},${whereOps})"$)
	End If
...
Sub findWhereOrderBy(whereMap As Map, whereOps As List, orderBy As List) As List
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.findWhereOrderBy(${BANano.ToJson(whereMap)},${whereOps},${orderBy})"$)
	End If
	If Schema.Size = 0 Then
		BANano.Throw($"SDUIRestApi.findWhereOrderBy. Please execute SchemaAdd?? first to define your table schema!"$)
		Return Null
	End If
	If UseApiKey Then
		If sApiKey = "" Then
			BANano.Throw($"SDUIRestApi.findWhereOrderBy - The ApiKey has not been specified!"$)
			Return Null
		End If
	End If
	'we build the filter
	...
	If scommand = "" Then
		fetch.SetURL($"/api/records/${TableName}"$)
		If ShowLog Then
			Log($"SDUIRestApi.findWhereOrderBy.(${baseURL}/records/${TableName})"$)
		End If
	Else
		fetch.SetURL($"/api/records/${TableName}?${scommand}"$)
		If ShowLog Then
			Log($"SDUIRestApi.findWhereOrderBy.(${baseURL}/api/records/${TableName}?${scommand})"$)
		End If
	End If
	fetch.NoCache = True
'	fetch.NoCors = True
...
Sub SetRecordAsIs(rec As Map)
	...
	DisplayValue = Record.GetDEfault(DisplayField, "")
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.SetRecordAsIs(${BANano.ToJson(Record)})"$)
	End If
End Sub

Sub ADD_ORDER_BY_UPDATED_AT As SDUIRestApi
	ADD_ORDER_BY("updated")
	Return Me
End Sub

Sub ADD_ORDER_BY_CREATED_AT As SDUIRestApi
...
Sub SELECT_RAW(query As String) As List
	result.Initialize
	If ShowLog Then
		Log($"SDUIRestApi.${TableName}.SELECT_RAW"$)
	End If
	If UseApiKey Then
		If sApiKey = "" Then
			BANano.Throw($"SDUIRestApi.SELECT_RAW - The ApiKey has not been specified!"$)
			Return result
		End If
	End If
	If ShowLog Then
	...
	fetch.SetContentTypeApplicationJSON
	fetch.NoCache = True
	fetch.SetURL($"/api/query"$)
	If ShowLog Then
		Log($"SDUIRestApi.SELECT_ALL.(${baseURL}/api/query/?${query})"$)
	End If
...
Sub SelectDistinctWhere(tblfields As List, tblWhere As Map, operators As List, orderBy As List) As List
	If Schema.Size = 0 Then
		Log($"SDUIRestApi.SelectDistinctWhere: '${TableName}' schema is not set!"$)
	End If
