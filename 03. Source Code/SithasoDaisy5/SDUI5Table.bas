...
#Event: ExpandRow (Row As Int, item As Map, Hidden As Boolean)
...
private Sub SetDefaults
	...
	CustProps.Put("Title", "Table")
	CustProps.Put("TitleVisible", True)
	CustProps.Put("Icon", "")
	CustProps.Put("IconColor", "")
	CustProps.Put("IconSize", "32px")
	CustProps.Put("IconVisible", False)
	CustProps.Put("ExcelTitle", "Table")
	CustProps.Put("PageSize", "a4")
	CustProps.Put("FontSize", "11")
	CustProps.Put("Unit", "pt")
	CustProps.Put("Orientation", "landscape")
	CustProps.Put("PageBreakRepeat", "")
	CustProps.Put("PdfTheme", "striped")
	CustProps.Put("ItemsPerPage", "10")
	CustProps.Put("ListViewMode", False)
	CustProps.Put("TrapRowClick", False)
	CustProps.Put("HasFilter", False)
	CustProps.Put("HasExpand", False)
	CustProps.Put("TableSize", "md")
	CustProps.Put("NormalCase", False)
	CustProps.Put("WrapHeadings", False)
	CustProps.Put("LowerCase", False)
	CustProps.Put("IsNormal", False)
	CustProps.Put("IsZebra", False)
	CustProps.Put("Pagination", True)
	CustProps.Put("BadgesOutlined", False)
	CustProps.Put("BadgesSize", "md")
	...
	CustProps.Put("HasAddnew", True)
	CustProps.Put("AddNewTooltip", "")
	CustProps.Put("HasSaveSingle", False)
	CustProps.Put("SaveSingleTooltip", "")
	CustProps.Put("HasDeleteSingle", False)
	CustProps.Put("DeleteSingleTooltip", "")
	CustProps.Put("HasRefresh", True)
	CustProps.Put("RefreshTooltip", "")
	CustProps.Put("HasDeleteAll", False)
	CustProps.Put("DeleteAllTooltip", "")
	CustProps.Put("HasToolbarUpload", False)
	CustProps.Put("UploadToolbarTooltip", "")
	CustProps.Put("HasToolbarDownload", False)
	CustProps.Put("DownloadToolbarTooltip", "")
	CustProps.Put("MultipleFiles", False)
	CustProps.Put("HasExportMenu", False)
	CustProps.Put("ExportToCsv", False)
	CustProps.Put("ExportToCsvTooltip", "")
	CustProps.Put("ExportToPdf", False)
	CustProps.Put("ExportToPdfTooltip", "")
	CustProps.Put("ExportToXls", False)
	CustProps.Put("ExportToXlsTooltip", "")
	CustProps.Put("HasHelp", False)
	CustProps.Put("HelpTooltip", "")
	CustProps.Put("HasBack", True)
	CustProps.Put("BackTooltip", "")
	CustProps.Put("HasGrid", False)
	CustProps.Put("GridTooltip", "")
	CustProps.Put("HasAlphaChooser", True)
	CustProps.Put("AlphaChooserTextColor", "#ffffff")
	CustProps.Put("AlphaChooserColumn", "name")
	CustProps.Put("AlphaChooserHeight", "8")
	CustProps.Put("HasColumnChooser", True)
	CustProps.Put("ColumnChooserTextColor", "#ffffff")
	CustProps.Put("ColumnChooserHeight", "8")
	CustProps.Put("ColumnChooserColor", "primary")
	CustProps.Put("ShowTotalRecords", True)
	CustProps.Put("HasEdit", True)
	CustProps.Put("EditTooltip", "")
	CustProps.Put("HasClone", False)
	CustProps.Put("CloneTooltip", "")
	CustProps.Put("HasDelete", True)
	CustProps.Put("DeleteTooltip", "")
	CustProps.Put("HasUpload", False)
	CustProps.Put("UploadTooltip", "")
	CustProps.Put("HasDownload", False)
	CustProps.Put("DownloadTooltip", "")
	CustProps.Put("HasMenu", False)
	CustProps.Put("MenuTooltip", "")
	CustProps.Put("FlexWrapActions", False)
...
Sub DesignerCreateView (Target As BANanoElement, Props As Map)
	...
	If Props <> Null Then
		...
		sTitle = Props.GetDefault("Title", "Table")
...
Sub setHasExpand(b As Boolean)
	bHasExpand = b
	CustProps.Put("HasExpand", bHasExpand)
End Sub

Sub getHasExpand As Boolean
	Return bHasExpand
End Sub
...
Sub AddDesignerColums					'ignoredeadcode
	...
	If bHasExpand Then AddColumnExpand("transparent")
End Sub
...
'add edit action
'<code>
'tb4.AddColumnExpand(app.COLOR_PRIMARY)
'</code>
Sub AddColumnExpand(color As String)
	AddColumnAction("expand", "", "./assets/chevron-down-solid.svg", color, "#000000")
	Dim tc As TableColumn = Columns.Get("expand")
	tc.ReplaceSvg = False
	tc.FitSvg = False
	tc.Ghost = True
...
'	End If
    ...
    <svg-renderer id="${mName}_${RowCnt}_${fldName}_icon"  data-js="enabled" style="${BuildIconColor(btnColor)};width:${iconsize};height:${iconsize}" data-src="${theicon}" fill="currentColor" class="${cClass}"></svg-renderer>
    </td>"$
	Return act
End Sub
...
'	End If
    ...
    <div><svg-renderer id="${mName}_${RowCnt}_${fldName}_icon"  fill="currentColor" data-src="${theicon}" width="${iconsize}" height="${iconsize}" style="${BuildIconColor(btnColor)};" data-js="enabled" class="${cClass}"></svg-renderer></div>
...
'	End If
    ...
    <div id="${mName}_${RowCnt}_${fldName}_flex" class="flex items-center">
    <div id="${mName}_${RowCnt}_${fldName}_subtitle" class="pl-1 pr-2 text-gray-700 text-base">${subtitle}</div>
    <div><svg-renderer id="${mName}_${RowCnt}_${fldName}_icon" data-js="enabled" fill="currentColor" data-src="${theicon}" height="${iconsize}" width="${iconsize}" style="${BuildIconColor(btnColor)};" class="${cClass}"></svg-renderer></div>
    </div>
    </td>"$
	Return act
End Sub
...
Private Sub BuildRowColor(Module As Object, fldName As String, fldValu As String, rowdata As Map, RowCnt As Int, tc As TableColumn) As String					'ignore
	...
	Dim act As String = $"[BANCLEAN]
    <td id="${mName}_${RowCnt}_${fldName}" class="${BuildClasses(tc)} ${tcolor} ${bgColor}" style="${BuildStyle(tc)}">
    <div id="${mName}_${RowCnt}_${fldName}_flex" class="inline-flex items-center">
...
Private Sub BuildRowAction(Module As Object, fldName As String, fldValu As String, rowdata As Map, RowCnt As Int, tc As TableColumn) As String			'ignore
	...
	If tc.ComputeClass <> "" Then
		...
		cClass = BANano.CallSub(Module, subName1, Array(rowdata))
	End If
	If tc.Ghost = True Then cClass = cClass & "btn-ghost"
	cClass = cClass.Trim
	'
	Dim subtitle As String = rowdata.GetDefault(tc.subtitle, "")
	subtitle = UI.CStr(subtitle)
...
Sub AddRow(rowdata As Map)
	...
	If bHasExpand Then
		...
		<tr id="${mName}_${RowCnt}_expandrow" class="hidden expandable-content">
			<td id="${mName}_${RowCnt}_expandcol" colspan="${numCols}" class="p-4 bg-base-200">
				<div id="${mName}_${RowCnt}_expanddata" class="w-full">
					Row ${RowCnt} Expand Data
				</div>
			</td>
		</tr>"$)
		Dim strRow As String = sbRow.tostring
	...
	For Each fldName As String In passwords.Keys
		...
		Else
			...
			elPrependIcon.SetData("src", tc.PrependIcon)
		...
		Else
			...
			elAppendIcon.SetData("src", tc.AppendIcon)
...
private Sub HandleClick(event As BANanoEvent)     'ignoredeadcode
	...
	Dim action As String = UI.MvField(src,3, "_")
	rowpos = UI.CInt(rowpos)
	'we are expanding a row/collapsing it
	If action = "expand" Then
		Dim iconKey As String = $"${mName}_${rowpos}_expand_icon"$
		Dim rowKey As String = $"${mName}_${rowpos}_expandrow"$
		Dim svgRenderer As BANanoElement = BANano.GetElement($"#${iconKey}"$)
		Dim svgEl As BANanoObject = svgRenderer.GetField("shadowRoot").RunMethod("querySelector", "svg")
		Dim curTransform As String = svgEl.GetField("style").GetField("transform").result
		curTransform = UI.CStr(curTransform)
		
		Select Case curTransform
		Case "rotate(180deg)"
			svgEl.GetField("style").SetField("transform", "rotate(0deg)")
		Case Else
			svgEl.GetField("style").SetField("transform", "rotate(180deg)")
		End Select
		svgEl.GetField("style").SetField("transition", "transform 0.3s ease")
		BANano.Await(BANano.GetElement($"#${rowKey}"$).ToggleClass("hidden"))
		Dim bHasHidden As Boolean = BANano.GetElement($"#${rowKey}"$).HasClass("hidden")
		'
		If SubExists(mCallBack, $"${mName}_expandrow"$) Then
			rowpos = BANano.parseInt(rowpos) - 1
			Dim Rowx As Map = GetRow(rowpos)
			Dim rowc As Map = BANano.DeepClone(Rowx)
			BANano.CallSub(mCallBack, mName & "_expandrow", Array(rowpos, rowc, bHasHidden))
		End If
		Return
	End If
	'
...
'return the expandrow id
Sub GetExpandRowID(pos As Int) As String
	Dim skey As String = $"${mName}_${pos}_expandrow"$
	Return skey
End Sub

'return the expandrow id
Sub GetExpandColumnID(pos As Int) As String
	Dim skey As String = $"${mName}_${pos}_expandcol"$
	Return skey
End Sub

'return the expandrow id
Sub GetExpandColumnDataID(pos As Int) As String
