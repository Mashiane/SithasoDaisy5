...
function compressBase64(base64Str) {
    const strBytes = new TextEncoder().encode(base64Str);
    const compressed = pako.deflate(strBytes);
    return btoa(String.fromCharCode(...compressed)); // convert to base64
};
function decompressBase64(compressedBase64Str) {
    const binaryStr = atob(compressedBase64Str);
    const compressedBytes = Uint8Array.from(binaryStr, c => c.charCodeAt(0));
